...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	//If sound bite is full print error message
	if(soundBitesFull) {
		printf("Error: unable to add sound because soundBites is full\n");
	}
}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	//Wipe the playbackBuffer to all 0's.
	memset(playbackBuffer, 0, 2 * size);
		...
		pthread_mutex_lock(&audioMutex);
		{
			if(soundBites[i].pSound != NULL) {
				...
				for(int j = 0; j < size; j++) {
					...
					short pcm = ((soundBites[i].pSound)->pData)[offset];

					//https://vladris.com/blog/2018/10/13/arithmetic-overflow-and-underflow.html
					if((playbackBuffer[j] >= 0) && (pcm > SHRT_MAX - playbackBuffer[j])) {//Check for overflow
						playbackBuffer[j] = SHRT_MAX;
					} else if((playbackBuffer[j] < 0) && (pcm < SHRT_MIN - playbackBuffer[j])) { //check for underflow
						playbackBuffer[j] = SHRT_MIN;	
					} else {
						playbackBuffer[j] += pcm;
					}
